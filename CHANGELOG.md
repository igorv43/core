...
* (app) Liquidity Fabric Part II — upgrade `v18`: perpetuals module `x/perp` (isolated-margin positions novated with the protocol through the x/batch margin hook, insurance fund waterfall with ADL, oracle-state risk engine with gradual steps, resident trigger orders and auto top-up, listing criteria enforced in code, allocation cascade with settlement transfers to the Oracle and Community Pools and a buyback burned through the internal spot market). No market is created by the upgrade. x/batch gains the perpetual order class (`Intent.reduce_only`, `SubmitPerpIntent`).
