...
* (remote) Liquidity Fabric D-30, D-31 and D-33 (spec v0.9.9, in `v19`): conversion receipts and converted deposits from the enrolled gateway, withdrawals to deterministic exit contracts (CREATE2) with confirmations; FabricExecutor control by consensus (rebalancing epoch, dynamic fees, ABI control orders with nonce; governance-only leg enrolment and pauses); CCTP ports of entry (accounts derived from the port domain, withdrawals routed to the vault's exit by the port sentinel); the remote module burns the dynamic withdrawal fee of the settlement asset.
* (warpledger) Liquidity Fabric D-29 (spec v0.9.7 §11.4, in `v15`): settlement basket policy — `MsgSetBasketToken` marks a synthetic warp token as a multi-origin basket; `settle_source_cap` (0.40) and per-origin `MsgSetOriginPolicy` (share cap override, pause) bound what each origin may back, enforced before `ProcessMessage` so a refused deposit stays a pending message; paused origins refuse deposits and redemptions; the ledger query exposes the composition. Simulation packages for the six Fabric modules and the `TestFabric` interchaintest.
* (app) Liquidity Fabric Part II — upgrade `v19`: `x/remote` (Hyperlane app 3: accounts derived from the origin sender controlled by verified payloads of a closed message whitelist, session keys via x/authz, paymaster via x/feegrant, withdrawals locked to the controller through warp) and `x/ismbond` (LUNC bonds of ISM operators, root history of outbound messages recorded by the warp wrapper, permissionless evidence of equivocation or invalid outbound checkpoints slashing the bond, pro-rata rewards). `x/warpledger` gains `bonded_cap_threshold`: caps above it require a bonded ISM. The app's x/feegrant keeper now has its bank keeper set.
* (app) Liquidity Fabric Part II — upgrade `v18`: perpetuals module `x/perp` (isolated-margin positions novated with the protocol through the x/batch margin hook, insurance fund waterfall with ADL, oracle-state risk engine with gradual steps, resident trigger orders and auto top-up, listing criteria enforced in code, allocation cascade with settlement transfers to the Oracle and Community Pools and a buyback burned through the internal spot market). stLUNC accepted as collateral under the rules of spec §21.5 (haircut with queue term, share and global caps, separate fund tranche redeemed through x/liquidstake and sold in the internal spot market; closed at the upgrade). No market is created by the upgrade. x/batch gains the perpetual order class (`Intent.reduce_only`, `SubmitPerpIntent`).
* (app) Liquidity Fabric Part II — upgrade `v17`: sealed-bid uniform-price call auction module `x/batch` (escrowed limit intents, bonded solvers with commit/reveal and objective slashing, delivery-versus-payment settlement inside the oracle price band, integrator builder codes, community-pool fee sink until the perpetuals insurance fund exists). The upgrade creates no market; governance registers markets with `MsgCreateMarket`.
* (oracle) Liquidity Fabric phase 8 stage 2 (in `v17`): assets priced directly in USD by the same ballot (`asset_whitelist` parameter, `<price><asset>@<depth>` vote tuples committing the Depth2%, tally without cross rates, `AssetPrice`/`AssetPrices`/`AssetTargets` queries, `Keeper.GetPrice`). The upgrade writes an empty asset whitelist.
* (oracle) Liquidity Fabric phase 7 stage 1: per-period ballot dispersion and standard deviation persisted with a bounded rate history; `Dispersion`, `Twap` and `RateHistory` queries (no store migration).
